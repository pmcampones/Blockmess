...
public class LazyPushBroadcast extends GenericProtocol implements BroadcastProtocol {
	...
	/**
	 * Identifiers of the messages whose content this node is waiting for, mapped to the peers that have the message
	 * content.
	 */
	...
	/**
	 * Maps the identifiers of objects that block in the broadcast to the identifiers of the messages that encapsulated
	 * them.
	 ...
	 * <p>&emsp 1st -> node A receives the contents of a value proposed by the application and notifies its peers
	 * sending them the value identifier</p>
	 * <p>&emsp 2nd -> node B receives the identifier and checks if it has a value with the same identifier. If not,
	 * requests the value from A
	 * <p>&emsp 3rd -> node A receives the request from B and sends it the value.</p>
	 * <p>Upon the conclusion of these steps, node B will repeat the process from step 1 to transmit the information to
	 * some other node C.</p>
	...
	/**
	 * This node records a received message 'msg', delivers it to the application, and notifies all its peers of the
